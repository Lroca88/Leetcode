...
public class MostVisitedSector {
    public IList<int> MostVisited(int n, int[] rounds) {
        // I just need the first and last element of the rounds
        int start = rounds[0]; int end = rounds[rounds.Length - 1];
        List<int> mostVisited = new List<int>();
        // If starts < end is just adding the elements
        if (start <= end) {
            for (int i = start; i <= end; i++) 
                mostVisited.Add(i);
        } else {
            // Otherwise, we need to get smaller items including end
            for (int i = 1; i <= end; i++) 
                mostVisited.Add(i);
            // And we need bigger items including start
